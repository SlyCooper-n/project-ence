...
export const Navbar = ({
  ...
  return (
    <nav
      className={`py-5 sm:py-8 text-black z-50 ${
        translucent ? "bg-opacity-70 bg-base-100" : ""
      } ${
        absolute ? "sm:absolute sm:top-0 sm:left-0 sm:w-full" : ""
      } ${className}`}
    >
      <Container className="flex flex-row-reverse sm:flex-row justify-between items-center">
        {/* menu bar */}
        <button
          type="button"
        ...
        >
          <Container asChild>
            <motion.ul className="px-[2.5vw] flex flex-col justify-center gap-4 sm:gap-16">
              {menuOptions.map((opt) => {
                if (
                  router.pathname.includes("/budget") &&
